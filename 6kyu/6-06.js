...
Array.prototype.under = function (under) {
  let res = [];
  for (let i = 0; i < this.length; i++) {
    if (Number.isInteger(this[i]) && this[i] < under) {
      res.push(this[i]);
    }
  }
  return res;
...
Array.prototype.over = function (over) {
  let res = [];
  for (let i = 0; i < this.length; i++) {
    if (Number.isInteger(this[i]) && this[i] > over) {
      res.push(this[i]);
    }
  }
  return res;
...
Array.prototype.inRange = function (a, b) {
  let res = [];
  for (let i = 0; i < this.length; i++) {
    if (Number.isInteger(this[i]) && this[i] >= a && this[i] <= b) {
      res.push(this[i]);
    }
  }
  return res;
